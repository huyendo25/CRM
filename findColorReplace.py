...
def color_string(key, countKey, p1, p):
    ...
    for substring in substrings[:-1]:
        ...
        p.add_run(substring)
        font = p.add_run(
            key).font.highlight_color = WD_COLOR_INDEX.YELLOW  # tô vàng key
        count = str(countKey)
        # tô đỏ số thứ tự của key
        font = p.add_run(count).font.highlight_color = WD_COLOR_INDEX.RED
...
def replace_string(key, value, numberList, countKey, p):
    ...
    for i in range(len(line_split)):
        # nếu từ đầu trong key xuất hiện
        if re.search(key_split[0], line_split[i], re.IGNORECASE):
            count = 0  # đếm từ trong key
            while count < len_key:
                # so khớp không phân biệt hoa thường
                if re.search(key_split[count], line_split[i+count], re.IGNORECASE):
            ...
            if count == len_key:  # nếu đủ
                ...
                if countKey in numberList:  # bắt đầu thay đổi ở các vị trí cần thiết
                    ...
                    while count_1 < len_key:
                        # thêm u ở phía trước để xử lý ký tự tiếng việt nhá
                        line_split[i+count_1] = u""
                        count_1 += 1
    ...
    for block in iter_block_items(doc):
        if isinstance(block, Paragraph):
            if re.search(key, block.text, re.IGNORECASE):  # so khớp không phân biệt hoa thường
                countKey = replace_string(
                    key, value, numberList, countKey, block)
        else:
            for row in block.rows:
                for p in row.cells:
                    # so khớp không phân biệt hoa thường
                    if re.search(key, p.text, re.IGNORECASE):
                        countKey = replace_string(
                            key, value, numberList, countKey, p)
